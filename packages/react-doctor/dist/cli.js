...
const BRANDING_LINE = `React Doctor Local ${highlighter.dim("by Veonum.com")}`;
...
const printSummary = (diagnostics, elapsedMilliseconds, scoreResult, totalSourceFileCount, noScoreMessage) => {
...
const runScan = async (directory, options, userConfig, startTime) => {
	...
	printSummary(diagnostics, elapsedMilliseconds, scoreResult, isDiffMode ? includePaths.length : lintSourceFileCount, noScoreMessage);
	if (hasSkippedChecks) {
		const skippedLabel = skippedChecks.join(" and ");
		logger.break();
