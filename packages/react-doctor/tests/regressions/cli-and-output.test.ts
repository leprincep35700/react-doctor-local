 ...
 *   Local-only fork — scan output never includes remote share links
...
describe("local-only output", () => {
  it("never prints a remote share link, even when offline is false", async () => {
    const projectDir = setupMinimalReactProject("local-only-no-share-link");
    writeFile(
...
`,
    ...
    const run = await captureScanOutput(projectDir, { offline: false });

    expect(run.stdout).not.toContain("Share your results");
    expect(run.stdout).not.toContain("react.doctor/share");
  });
});
